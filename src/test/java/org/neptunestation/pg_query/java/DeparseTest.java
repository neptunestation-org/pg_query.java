...
public class DeparseTest {
	...
	for (JsonValue j : Json.createReader(getClass().getResourceAsStream("/deparse_tests.json")).readObject().getJsonArray("tests")) {
	    System.out.println(j.asJsonObject().getJsonString("id").getString());
	    System.out.println(j.asJsonObject().getJsonString("sql").getString());
	    System.out.println(PgJava.pg_query_deparse_protobuf(PgJava.pg_query_parse_protobuf(j.asJsonObject().getJsonString("sql").getString()).getParse_tree()).getQuery());
